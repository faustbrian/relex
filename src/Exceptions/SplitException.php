...
/**
 * Exception thrown when a regex split operation fails.
 * @author Brian Faust <brian@cline.sh>
 */
final class SplitException extends RelexException
{
    /**
     * Create exception for a failed split operation.
     */
    public static function failed(string $pattern, string $subject, string $error): self
    {
        return new self(
            sprintf(
                'Error splitting subject "%s" with pattern "%s": %s',
